...
// Removed useEffect since we don't need it to call a custom hook

export function ChatInput({ 
  indexedRepos, loading, repoFilter, toggleFilter, isDark, textareaRef, input, 
  setInput, handleKey, sendMessage, isTyping, card, 
  border, inputBg, text, muted 
}: any) {
  
  // 1. Call the hook at the top level of the component

  // 2. Check if we have any repos to enable/disable the input
  const hasConnectedRepos = indexedRepos.length > 0;
      ...
      <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-2.5">
            {repoFilter.map((r: string) => (
              <span key={r} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg" style={{ background: "rgba(124,111,247,0.12)", color: "#a89ff9", border: "1px solid rgba(124,111,247,0.2)", fontFamily: "monospace" }}>
                {r}
                <button onClick={() => toggleFilter(r)} className="ml-0.5 hover:text-red-400 transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </span>
            ))}
        ...
        <div className="flex items-end gap-2 rounded-2xl px-3 py-2.5" style={{ background: inputBg, border: `1.5px solid ${border}` }}>
          <div className="flex-shrink-0 self-end pb-0.5">
            {/* 3. Pass indexedRepos to the dropdown */}
            <RepoDropdown repos={indexedRepos} selected={repoFilter} onToggle={toggleFilter} isDark={isDark} />
          </div>
          <textarea
            ref={textareaRef}
            value={input}
            ...
            onKeyDown={handleKey}
            // 4. Update placeholder and disabled states based on the hook's data
            placeholder={
              loading ? "Loading repositories..." 
              : !hasConnectedRepos ? "Connect a repo first…" 
              : "Ask anything about your code…"
            }
            disabled={!hasConnectedRepos || loading}
            rows={1}
          ...
          />
          <button onClick={sendMessage} disabled={!input.trim() || isTyping || !hasConnectedRepos}
        ...
        <p className="text-center text-[11px] mt-2.5 mb-1" style={{ color: muted }}>
          AI can make mistakes. Please verify important code and information.
        </p>
      </div>
    </div>
  );
