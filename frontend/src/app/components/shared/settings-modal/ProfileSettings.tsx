export function ProfileSettings({user,repos, text, muted, cardBg, border }: any) {
  console.log(user)
  return (
    <>
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white flex-shrink-0" style={{ background: "linear-gradient(135deg,#7c6ff7,#5b8def)" }}>
          <img src={user.avatar_url} alt="" />
        </div>
        <div>
          <p className="font-semibold" style={{ color: text }}>{user.github_username}</p>
          <p className="text-sm" style={{ color: muted }}>{user.email}</p>
          <p className="text-xs mt-1 font-mono" style={{ color: "#7c6ff7" }}>github.com/{user.github_username}</p>
        </div>
      </div>
      {/* {[["Display Name",user.github_username], ["Email", user.email]].map(([label, val]) => (
        <div key={label}>
      ...
      <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: cardBg }}>
        <div>
          ...
          <p className="text-xs" style={{ color: muted }}>{repos} repos synced</p>
        </div>
